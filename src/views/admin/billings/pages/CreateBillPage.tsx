...
/* -------------------------
   ...
   We compute in paise (integer) to avoid floating point drift.
   ------------------------- */
...
/* -------------------------
   Form schema (zod)
   discount is stored in paise (integer). Items contain item id and qty.
   ------------------------- */
const billSchema = z.object({
    phone: z.string().min(10, 'Phone number must be at least 10 digits'),
    discount: z.number().min(0).optional(), // paise
...
const CreateBillPage: React.FC = () => {
    ...
    // watch items and discount type/value
    ...
    /* -------------------------
       Typeahead: Debounced phone search with cancellation token
       - Only search when query length >= 3
       ------------------------- */
    useEffect(() => {
        // tie UI phoneQuery to form phone field
        setPhoneQuery(watchedPhone ?? '');
    }, [watchedPhone]);

    useEffect(() => {
        if ((phoneQuery?.length ?? 0) < 3) {
            setUserSuggestions([]);
        ...
        searchDebounceRef.current = window.setTimeout(async () => {
            ...
            try {
                ...
                // only take result if token matches last token (simple cancel)
    ...
    useEffect(() => {
        const onKey = (e: KeyboardEvent) => {
            if (!isSuggestionsOpen || userSuggestions.length === 0) return;
            if (e.key === 'ArrowDown') {
                e.preventDefault();
                suggestionIndexRef.current = Math.min(suggestionIndexRef.current + 1, userSuggestions.length - 1);
                ...
                if (suggestionIndexRef.current >= 0 && suggestionIndexRef.current < userSuggestions.length) {
                    const sel = userSuggestions[suggestionIndexRef.current];
                    selectSuggestion(sel.phone);
                }
        ...
        // eslint-disable-next-line
    }, [isSuggestionsOpen, userSuggestions]);

    const scrollSuggestionIntoView = () => {
        const container = suggestionsRef.current;
        if (!container) return;
        const idx = suggestionIndexRef.current;
        const el = container.querySelectorAll('[data-suggestion-index]')[idx] as HTMLElement | undefined;
    ...
    /* -------------------------
       Derived money calculations (memoized):
       subtotalPaise, discountPaise (derived), grandTotalPaise
       Uses itemsList lookup and watchedItems. All arithmetic done in paise.
       ------------------------- */
    const subtotalPaise = useMemo(() => {
        // for each item in watchedItems, find price; if missing treat as 0
        return watchedItems.reduce((accPaise, it) => {
            const selected = itemsList.find(i => i._id === it.item);
            if (!selected) return accPaise;
            const unitPaise = toPaise(Number(selected.price));
            const qty = Number(it.quantity) || 0;
            return safeAddPaise(accPaise, safeMulPaise(unitPaise, qty));
        }, 0);
    }, [watchedItems, itemsList]);

    const [discountInputValue, setDiscountInputValue] = useState<string>('0'); // string shown in input
    useEffect(() => {
    ...
    useEffect(() => {
        if (discountInputValue === '' || discountInputValue === undefined) {
        ...
        if (watchedDiscountType === 'percentage') {
            ...
            const discountPaise = safeMinPaise(Math.round((subtotalPaise * boundedPercent) / 100), subtotalPaise);
            setValue('discount', discountPaise, { shouldDirty: true });
        } else {
            const amount = Math.max(0, Number(discountInputValue) || 0);
            const amountPaise = toPaise(amount);
            const bounded = safeMinPaise(amountPaise, subtotalPaise);
            setValue('discount', bounded, { shouldDirty: true });
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [discountInputValue, subtotalPaise, watchedDiscountType]);
    ...
    const handleAddItem = useCallback(() => {
        append({ category: '', item: '', quantity: 1 });
    }, [append]);

    const handleRemoveItem = useCallback((index: number) => {
        // remove via useFieldArray remove ensures the form array state is consistent
    ...
    const onSubmit = async (values: BillForm) => {
        // prevent creating when some item ids are empty
        ...
        try {
            ...
            if (res?.status === 1) {
                navigate('/bill-preview', { state: { billData: res.data } });
            } else {
                // handle error (toast/log)
                console.error('create bill failed', res);
            }
        } catch (err) {
    ...
    /* -------------------------
       Small UI helper for payment icons
       ------------------------- */
        ...
        <div>
            <form className="grid grid-cols-1 lg:grid-cols-3 gap-6" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2"><Phone className="h-5 w-5 text-primary" /> Customer</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="relative">
                                ...
                                {isSuggestionsOpen && userSuggestions.length > 0 && (
                                    <div
                                        ref={suggestionsRef}
                                        id="phone-suggestions"
                                        className="absolute z-50 mt-2 max-h-48 w-full overflow-auto rounded-md border bg-white shadow-lg"
                                        role="listbox"
                                    >
                                        {userSuggestions.map((s, idx) => (
                                            <div
                                                ...
                                                role="option"
                                                aria-selected={idx === suggestionIndexRef.current}
                                                className={`cursor-pointer px-3 py-2 ${idx === suggestionIndexRef.current ? 'bg-gray-100' : 'hover:bg-gray-50'}`}
                                                onMouseDown={(e) => { e.preventDefault(); selectSuggestion(s.phone); }} // use onMouseDown to avoid blur
                                            >
                                                <div className="flex justify-between items-center">
                                                    <div className="text-sm font-medium">{s.phone}</div>
                                                    {/* show badge if needed e.g. existing customer */}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        <CardContent className="space-y-4">
                            {fields.map((field, index) => {
                                ...
                                const categories = Array.from(
                                    new Map(itemsList.map(it => [it.category._id, it.category])).values()
                                );
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {/* Item Select (filtered by category) */}
                                            ...
                                            <Controller
                                                ...
                                                render={({ field: itemField }) => (
                                                    <Select
                                                        ...
                                                        disabled={!getValues(`items.${index}.category`)}
                                                    ...
                                                    >
                                                        ...
                                                        <SelectContent>
                                                            {itemsList
                                                                .filter(it => it.category._id === getValues(`items.${index}.category`))
                                                                .map(it => (
                                            ...
                                            <Controller
                                                ...
                                                render={({ field: qtyField }) => (
                                                    <Input
                                                        ...
                                                        {...qtyField} // eslint-disable-next-line
                                                        onChange={(e: any) => {
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div className="flex gap-2">
                                    <Button
                                        type="button"
                                        variant={watchedDiscountType === 'percentage' ? 'default' : 'solid'}
                                        size="sm"
                                    ...
                                    <Button
                                        type="button"
                                        variant={watchedDiscountType === 'amount' ? 'default' : 'solid'}
                                        size="sm"
                                ...
                                <Input
                                    ...
                                    min={0}
                                    max={watchedDiscountType === 'percentage' ? 100 : undefined} // eslint-disable-next-line
                                    onChange={(e: any) => {
                        ...
                        <CardContent className="space-y-4">
                            <div className="space-y-3">
                                {watchedItems.map((it, idx) => {
                                    ...
                                    const unitPaise = toPaise(Number(selected.price));
                                    const linePaise = safeMulPaise(unitPaise, Number(it.quantity) || 0);
                                    return (
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                    <span>Subtotal:</span>
                                    <span>₹{fromPaise(subtotalPaise).toFixed(2)}</span>
                                </div>
                                {discountPaise > 0 && (
                                    <div className="flex justify-between text-success">
                                        <span>Discount:</span>
                                        <span>-₹{fromPaise(discountPaise).toFixed(2)}</span>
                                    </div>
                                )}
                                <Separator />
                                <div className="flex justify-between text-lg font-bold">
                                    <span>Total:</span>
                                    <span>₹{fromPaise(grandTotalPaise).toFixed(2)}</span>
                                </div>
                            </div>
                            ...
                            <Button
                                type="submit"
                                className="w-full"
                                disabled={isLoading || watchedItems.some(it => !it.item)}
                            >
